...
- **问题集**
    - 修复未开始或准备中的题集在资源管理器中触发 403 错误的问题（[Issue #22](https://github.com/jinzcdev/vscode-pintia/issues/22)）
    - 静默处理预期的 403 响应，避免懒加载反复重试导致资源管理器不可用
...
- **文件命名**（[Issue #14](https://github.com/jinzcdev/vscode-pintia/issues/14)）
    - 支持自定义文件名模板，包括题目 ID、题目标签、题目名称、题目集 ID 等
    - 支持使用拼音转换中文题目名称
    - 支持自定义题目集文件夹名称
- **编辑器快捷键**
    - 在 PTA 源文件中新增提交答案（`Cmd/Ctrl+Shift+J`）与测试代码（`Cmd/Ctrl+Shift+K`）的快捷键
    - 在题目预览页增加 **开始编程**（`Cmd/Ctrl+Enter`）快捷键
