...
export async function showUserManager(): Promise<void> {
    ...
    switch (choice.value) {
        case "HOME":
            vscode.env.openExternal(vscode.Uri.parse("https://pintia.cn/home/account"));
            break;
        case "REGISTER":
            vscode.env.openExternal(vscode.Uri.parse("https://pintia.cn/auth/register"));
            break;
        case "CHECKIN":
            vscode.commands.executeCommand("pintia.checkIn");
            break;
        case "SIGNIN":
            vscode.commands.executeCommand("pintia.signIn");
            break;
        case "SIGNOUT":
            vscode.commands.executeCommand("pintia.signOut");
            break;
    }
}
...
export async function checkInPTA(): Promise<void> {
    try {
        ...
        promptForOpenOutputChannel(
            l10n.t("Failed to check in. Please check the output channel for details."),
            DialogType.error
        );
    }
}
...
export async function checkedInStatus(): Promise<boolean> {
    try {
        ...
        ptaChannel.error(error.toString());
        promptForOpenOutputChannel(
            l10n.t("Failed to get the status of checkin. Please check the output channel for details."),
            DialogType.error
        );
    }
    return false;
