...
export async function createProblemSearchIndex(context: vscode.ExtensionContext) {
    ...
    if (!(await fs.pathExists(indexFileDest))) {
        ...
        try {
            ...
            await promptForOpenOutputChannel(
                l10n.t("Failed to copy problem index. Please check the output channel for details."),
                DialogType.error
            );
        }
    }
    ...
    await vscode.window.withProgress(
        ...
        async (p: vscode.Progress<{ message?: string; increment?: number }>) => {
            return new Promise<void>(async (resolve: () => void, reject: (e: Error) => void): Promise<void> => {
                ...
                try {
                    const problemSets: IProblemSet[] = await ptaApi.getAlwaysAvailableProblemSets(
                    ...
                    if (Object.keys(problemIndex).length !== 0) {
                        ...
                        await promptForOpenOutputChannel(
                            l10n.t(
                                "Failed to fetch problem search index. Please check the output channel for details."
                            ),
                            DialogType.error
                        );
                    }
