    ...
    try {
        ...
        if (problemCompiler !== "NO_COMPILER" && defaultCompiler !== problemCompiler) {
            vscode.window.showInformationMessage(
                l10n.t("Only {0} is allowed in this problem.", compilerLangMapping.get(problemCompiler) ?? UNKNOWN)
            );
        ...
        if (!(await fs.pathExists(finalPath))) {
            ...
            const problem: IProblem = await ptaApi.getProblem(
                ptaCode.psID,
                ptaCode.pID,
            ...
            const lastSubmissionDetail = (
                await ptaApi.getLastSubmissions(ptaCode.psID, ptaCode.pID, ptaManager.getUserSession()?.cookie ?? "")
            )?.submissionDetails[0];
            const lastSubmittedCompiler: string =
                (
                    lastSubmissionDetail?.programmingSubmissionDetail ??
                    lastSubmissionDetail?.codeCompletionSubmissionDetail
                )?.compiler ?? problemCompiler;
            const lastProgram: string | undefined =
                lastSubmissionDetail && defaultCompiler === lastSubmittedCompiler
        ...
        await promptForOpenOutputChannel(
            l10n.t("Coding the problem failed. Please check the output channel for details."),
            DialogType.error
        );
    }
}
...
export async function searchProblem(): Promise<void> {
    ...
    const choice: IQuickPickItem<IProblemSearchItem> | undefined = await vscode.window.showQuickPick(items, {
        matchOnDetail: true,
...
async function fetchProblemIndex(): Promise<Array<IProblemSearchItem>> {
    ...
    try {
        ...
        await promptForOpenOutputChannel(
            l10n.t("Failed to fetch the problem search index. Please check the output channel for details."),
            DialogType.error
        );
    }
...
async function parseProblemsToPicks(
    p: Promise<IProblemSearchItem[]>
): Promise<Array<IQuickPickItem<IProblemSearchItem>>> {
    let cnt = 0;
    return new Promise(async (resolve: (res: Array<IQuickPickItem<IProblemSearchItem>>) => void): Promise<void> => {
        const picks: Array<IQuickPickItem<IProblemSearchItem>> = (await p).map((problem: IProblemSearchItem) =>
            Object.assign(
                {},
                {
